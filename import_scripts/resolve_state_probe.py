...
def main() -> None:
    ...
    if resolve_handle is None:
        print("Could not connect to Resolve. Is DaVinci Resolve running?")
        print("If using Resolve Free, run this file from Workspace > Scripts inside Resolve.")
        return

    snapshot = get_project_state_snapshot(resolve_handle)
    if snapshot is None:
        print("Connected, but no project or timeline is open.")
        return

    print("Resolve state snapshot:")
